...
// НЕ вызываем adaptCenterButtonsHeight() здесь - это вызывает баг с кнопками
}
...
if (!btn) return;
// ИСПРАВЛЕНО: не переопределяем padding и lineHeight, только высоту
btn.style.height = btnH + 'px';
// Убрали сброс padding - оставляем CSS значения по умолчанию
btn.style.display = 'flex';
btn.style.alignItems = 'center';
btn.style.justifyContent = 'center';
btn.style.boxSizing = 'border-box';
});
}, 150); // Увеличили задержку для стабильности
} catch (e) {
...
padding: 0;
    ...
    #${OVERLAY_ID} .overlay-header {
        ...
        padding: 0 8px;
        background: #323232;
        border-bot tom: 1px solid #3f3f3f;
        cursor: move;
    }
    ...
    /* Логотип игры нав ерху (в блоке игры); если нет URL — показывается название */
    ...
    #${OVERLAY_ID} .overlay-g rid {
        display: grid;
        grid-template-columns: 2fr 1fr 2fr;
        gap: 8px;
        align-items: stretch;
        flex: 1; /* занять всё доступное пространство над с трокой статуса */
    }
    ...
    #${OVERLAY_ ID} .qa-copy-btn:disabled {
        opacity: 0.4;
        cursor: default;
    }
    ...
    #${OVERLAY_ID} .center-bt n {
        ...
        /* ИСПРАВЛЕНО: добавили white-space и overflow для защиты от вылезания текста */
        white-space: nowrap;
        overflow: hidden;
        text-overflow: ellipsis;
    }
    ...
    #${OVERLA Y_ID} .center-btn:disabled {
        opacity: 0.5;
        cursor: default;
    }
    ...
    /* Скроллбар (ограничен классом внутри о верлея) */
...
if (!q) {
    ...
    // НЕ вызываем adaptCenterButtonsHeight() здесь - это вызывает баг при первом нажатии
    return;
}
...
setQuestionLoading(false);
// НЕ вызываем adaptCenterButtonsHeight() здесь - оставляем кнопки в исходном состоянии
}
function detectGame() {
...
try {
    ...
    // Адаптируем высоты кнопок только после поиска ответа (когда контент стабилен)
...
if (dom.deleteBtn) dom.deleteBtn.textContent = t.clearBtn;
    ...
    if (dom.questionCopyBtn) dom.questionCopyBtn.t itle = t.copyBtn;
    if (dom.answerCopyBtn) dom.answerCopyBtn.title = t.copyBtn;

    const minBtn = document.getElementById('minimize-btn');
    const closeBtn = document.getElem entById('close-btn');
    if (minBtn) minBtn.title = t.minimize;
    if (closeBtn) closeBtn.title = t.close;

    if (dom.status.textContent === getText('loadingDB', 'en') || dom.s tatus.textContent === getText('loadingDB', 'ru')) {
    ...
    if (dom.questionText.textContent === getText('placeholderQuestion', 'en') || dom.questionTex t.textContent === getText('placeholderQuestion', 'ru')) {
        dom.questionText.textContent = t.placeholderQuestion;
    }
    if (dom.answerText.textContent === getText('placeh olderAnswer', 'en') || dom.answerText.textContent === getText('placeholderAnswer', 'ru')) {
        dom.answerText.textContent = t.placeholderAnswer;
    }
    if (dom.answerText.t extContent === getText('answerNotFound', 'en') || dom.answerText.textContent === getText('answerNotFound', 'ru')) {
...
setPopupContent(popupEl ? popupEl.querySelector('.jf-popup-body').textConten t : '', currentContentLang);
...
const overPopup = popupEl.getBoundingClientRec t();
...
if (dom.questi onCopyBtn) dom.questionCopyBtn.disabled = true;
if (dom.answerCopyBtn) dom.answerCopyBtn.disabled = true;
if (dom.deleteBtn) dom.deleteBtn.disabled = true;
if (dom.dbStatus) dom.db Status.className = 'db-status error';
...
if ( dom.deleteBtn) {
    dom.deleteBtn.addEventListener('click', () = > {
        ...
        if (dom.answerConfidence) dom.answerConfidence.text Content = '';
        if (dom.answerCopyBtn) dom.answerCopyBtn.disabled = true;
        if (dom.deleteBtn) dom.deleteBtn.disabled = true;
        setQuestionLoading(false);
         setAnswerLoading(false);
        // НЕ вызываем adaptCenterButtonsHeight() при очистке - оставляем исходные стили
    });
}

const langBtn = overlayEl.querySelector('#lang-flag-btn');
if (langBtn) {
    langBtn.addEventListener('cli ck', () = > {
        currentLang = currentLang === 'ru' ? 'en' : 'ru';
        updateAllText();
        if(isDatabaseLoaded) updateStatus('dbLoaded', 'success');
        log('Language switc hed to:', currentLang);
    });
}
...
// Функция очистки - снимает гл обальные обработчики и удаляет попап/стили
function cleanupOverlay() {
    try {
        if (popupDocClickHandler) {
            document.removeEventListener('click', popupDocClick Handler);
            popupDocClickHandler = null;
        }
        if (windowResizeHandler) {
            window.removeEventListener('resize', windowResizeHandler);
            w indowResizeHandler = null;
        }
        hidePopup();
...
if (minBtn) minBtn.addEventListen er('click', () = > overlayEl.classList.toggle('overlay-minimized'));
...
function onMouseMove(e) {
    ...
    con st maxTop = window.innerHeight - rect.height;

    nextLeft = Math.max(0, Math.min(maxLeft, nextLeft));
    nextTop = Math.max(0, Math.min(maxTop, nextTop));

    overlayEl.style.l eft = nextLeft + 'px';
...
function endDrag() {
    if (!isDraggin g) return;
    isDragging = false;
    document.body.style.userSelect = '';
    window.removeEventListener('mousemove', onMouseMove);
    window.removeEventListener('mouseup', endD rag);
}
...
windowResizeHandler = () = > {
    updateIconSize();
    // НЕ вызываем adaptCenterButtonsHeight() при resize - это может ломать кнопки
};
